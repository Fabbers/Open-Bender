...
pulseWidth = 0.1; #100 microseconds
pulseDelay = 1; #1000 microseconds
...
def setup():
	...
	GPIO.setup(minSwitch, GPIO.IN) 
	GPIO.setup(maxSwitch, GPIO.IN)
...
def wait(mseconds):
	...
	while True:
		...
		if float(current_time - start_time) >= seconds/1000:
			break
...
def bendWire(anlge):
	if angle != 0:
		print "Bending to {} degrees".format(anlge)
		direction = cw
		reversedir = ccw
...
def homingRoutine():
	...
	while GPIO.input(minSwitch) != 0:
		rotatePin(cw, 1)
	
	print "Minimum limit switch reached"

	while GPIO.input(maxSwitch) != 0:
		rotatePin(ccw, 1)
		steps_count += 1
